...
囲碁3編、将棋1編、ローカル LLM 1編。囲碁の3編は単一の RTX 5090（Blackwell sm_120 / Core Ultra 9 285K, WSL2）による求解で、
...
ローカル LLM の1編は同じ 5090 の上で、LLM に文章を書かせず**選ばせるだけ**にすると
何が得られて何が得られないかを、特許のタグ付けで確かめたものです。
...
### [Decide, Don't Write](https://soy-tuber.github.io/nemotron/)

JEV は文章を生成せず、決められた選択肢を確率付きで返す「判断専用」のモデルです。
その考え方を、ローカルの Nemotron Nano 9B v2 Japanese（vLLM）の `logprobs` だけで再現しました。
1回の判定は **86 ms**、同時32件で毎秒36件。確信度は当たりやすさと連動し、
**0.9以上なら82%** が正解の目安（Gemini 3.8 Flash）と一致します。

クラウドで書かれた最初の版は、モックのテスト57件を通りながら**実機では1件も判定できません**でした。
思考オフの指定がこのモデルのテンプレートに効かない、vLLM が未知のフィールドを黙って無視する、
といった食い違いの記録も載せています。

特許の複数タグ付けに当てると、JEV 方式は当たりやすい一方でタグが減り、速くもなりませんでした。
効いたのは方式ではなく**入力**です。要約欄の先頭の25%が出願の定型文だったのを差し替えただけで、
タグが付かない率は **14.4% → 2.2%**、本番の「該当なし」は **35.3万件 → 2.5万件**になりました。
